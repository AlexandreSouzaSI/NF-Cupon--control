...
//
// Componente compartilhado (movido de components/product-sales/ pra cá em
// components/ui/) — usar em qualquer tela que escolha de uma lista longa
// ou dinâmica (itens de estoque, fornecedores, categorias, ingredientes,
// funcionários/freelancers etc). Selects de opção fixa e curta (perfil,
// forma de pagamento, unidade, status) continuam <select> normal.
export function AutocompleteInput({
    ...
    // handleBlur agenda um setTimeout — se nesses ~120ms o valor selecionado
    // mudar por fora (ex: o componente pai limpa a seleção depois de
    // adicionar o item à lista), uma closure comum leria o "value"/"options"
    // antigos, de quando o blur disparou, e reescreveria o campo com o
    // nome antigo por cima do que já tinha sido limpo — dando a impressão
    // de que o autocomplete "travou" no item anterior. Os refs abaixo
    // sempre guardam o valor mais atual pro timeout consultar.
    ...
    function handleBlur() {
        ...
        // do blur fechar a lista (senão o clique nunca registra). Lê dos
        // refs (não das variáveis "value"/"options" capturadas nesta
        // renderização) pra não sobrescrever com um valor já desatualizado
        // se o pai tiver limpado a seleção enquanto o timeout esperava.
        setTimeout(() => {
            setAberto(false);
            const atual =
                optionsRef.current.find((o) => o.id === valueRef.current) ||
                null;
            setQuery(atual?.nome || '');
        }, 120);
    }
    ...
    return (
        <div ref={containerRef} className="relative flex-1">
            <input
                ...
                className={
                    className ||
                    'w-full bg-transparent py-1.5 text-sm outline-none placeholder:text-zinc-400 disabled:opacity-50'
                }
                autoComplete="off"
            />
